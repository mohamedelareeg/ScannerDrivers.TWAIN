...
namespace ScannerDrivers.TWAIN
{
    public partial class TWAINSDK
    {
        ...
        public STS DatImagememfilexfer(DataGroups a_dg, Messages a_msg, ref TW_IMAGEMEMXFER a_twimagememxfer)
        {
            STS sts;
            ...
            if (this.m_runinuithreaddelegate == null)
            {
                if ((m_threadTwain != null) && (m_threadTwain.ManagedThreadId != Thread.CurrentThread.ManagedThreadId))
                {
                    lock (m_lockTwain)
                    {
                        ...
                        threaddata.dat = DataArgumentTypes.IMAGEMEMFILEXFER;
            ...
            if (Log.GetLevel() > 0)
            {
                Log.LogSendBefore(a_dg.ToString(), DataArgumentTypes.IMAGEMEMFILEXFER.ToString(), a_msg.ToString(), ImagememxferToCsv(a_twimagememxfer));
            }
            ...
            if (ms_platform == Platform.WINDOWS)
            {
                ...
                try
                {
                    if (m_threaddataDatImagememfilexfer.blIsInuse || (this.m_runinuithreaddelegate == null))
                    {
                        if (m_blUseLegacyDSM)
                        {
                            sts = (STS)NativeMethods.WindowsTwain32DsmEntryImagememfilexfer(ref m_twidentitylegacyApp, ref m_twidentitylegacyDs, a_dg, DataArgumentTypes.IMAGEMEMFILEXFER, a_msg, ref a_twimagememxfer);
                        }
                        else
                        {
                            sts = (STS)NativeMethods.WindowsTwaindsmDsmEntryImagememfilexfer(ref m_twidentitylegacyApp, ref m_twidentitylegacyDs, a_dg, DataArgumentTypes.IMAGEMEMFILEXFER, a_msg, ref a_twimagememxfer);
                        }
                    }
                    else
                    {
                        if (m_blUseLegacyDSM)
                        {
                            lock (m_lockTwain)
                            {
                                ...
                                m_threaddataDatImagememfilexfer.dat = DataArgumentTypes.IMAGEMEMFILEXFER;
            ...
            else if (ms_platform == Platform.LINUX)
            {
                ...
                try
                {
                    ...
                    {
                        sts = (STS)NativeMethods.Linux64DsmEntryImagememfilexfer(ref m_twidentitylegacyApp, ref m_twidentitylegacyDs, a_dg, DataArgumentTypes.IMAGEMEMFILEXFER, a_msg, ref a_twimagememxfer);
                    }
                    else if (m_blFoundLatestDsm && (m_linuxdsm == LinuxDsm.IsLatestDsm))
                    {
                        sts = (STS)NativeMethods.LinuxDsmEntryImagememfilexfer(ref m_twidentitylegacyApp, ref m_twidentitylegacyDs, a_dg, DataArgumentTypes.IMAGEMEMFILEXFER, a_msg, ref a_twimagememxfer);
                    }
                    else if (m_blFound020302Dsm64bit && (m_linuxdsm == LinuxDsm.Is020302Dsm64bit))
                    {
                        ...
                        sts = (STS)NativeMethods.Linux020302Dsm64bitEntryImagememfilexfer(ref m_twidentityApp, ref m_twidentityDs, a_dg, DataArgumentTypes.IMAGEMEMFILEXFER, a_msg, ref twimagememxferlinux64);
            ...
            else if (ms_platform == Platform.MACOSX)
            {
                ...
                try
                {
                    ...
                    if (m_blUseLegacyDSM)
                    {
                        sts = (STS)NativeMethods.MacosxTwainDsmEntryImagememfilexfer(ref m_twidentitymacosxApp, ref m_twidentitymacosxDs, a_dg, DataArgumentTypes.IMAGEMEMFILEXFER, a_msg, ref twimagememxfermacosx);
                    }
                    else
                    {
                        sts = (STS)NativeMethods.MacosxTwaindsmDsmEntryImagememfilexfer(ref m_twidentitymacosxApp, ref m_twidentitymacosxDs, a_dg, DataArgumentTypes.IMAGEMEMFILEXFER, a_msg, ref twimagememxfermacosx);
                    }
